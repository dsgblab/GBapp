<template>
    <AppLayout title="Tipos de documentos de identidad">
        <template #header>
            <div class="flex flex-row">
                ...
                <PrimaryButton type="button" class="ml-auto" @click="modal.open = true" v-permission="'design.time-state.create'">
...
export default {
    ...
    data(){
        return {
            table: {
                data: this.type_document_identifications
            },
    ...
    methods: {
        ...
        store(){
            ...
            if (this.v$.modal.form.$invalid){
                ...
                axios.post(route('type-document-identification.store'), this.modal.form).then(resp => {
                    this.closeModal()
        ...
        update(){
            ...
            if (this.v$.modal.form.$invalid){
                ...
                axios.put(route('type-document-identification.update', this.modal.form.id), this.modal.form).then(resp => {
                    this.closeModal()
                    this.table.data = resp.data
                }).catch(err => {
                    this.$swal({
        ...
        destroy(id){
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete(route('type-document-identification.destroy', id)).then(resp => {
                        this.table.data = resp.data
                    }).catch(err => {
                        this.$swal({
                            icon: 'error',
