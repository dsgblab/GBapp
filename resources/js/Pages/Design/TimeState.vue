<template>
    <AppLayout title="Diseño - Estados de tiempo">
        <template #header>
            <div class="flex flex-row">
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    Diseño - Estados de tiempo
                </h2>
    ...
    methods: {
        ...
        store(){
            ...
            if (this.v$.modal.form.$invalid){
                ...
                axios.post(route('time-state.store'), this.modal.form).then(resp => {
                    this.closeModal()
        ...
        update(){
            ...
            if (this.v$.modal.form.$invalid){
                ...
                axios.put(route('time-state.update', this.modal.form.id), this.modal.form).then(resp => {
                    this.closeModal()
                    this.table.data = resp.data
                }).catch(err => {
                    this.$swal({
        ...
        destroy(id){
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete(route('time-state.destroy', id)).then(resp => {
                        this.table.data = resp.data
                    }).catch(err => {
                        this.$swal({
                            icon: 'error',
