<template>
    <AppLayout title="Diseño - Prioridades">
        <template #header>
    ...
    methods: {
        ...
        store(){
            ...
            if (this.v$.modal.form.$invalid){
                ...
                axios.post(route('priority.store'), this.modal.form).then(resp => {
                    this.closeModal()
        ...
        update(){
            ...
            if (this.v$.modal.form.$invalid){
                ...
                axios.put(route('priority.update', this.modal.form.id), this.modal.form).then(resp => {
                    this.closeModal()
                    this.table.data = resp.data
                }).catch(err => {
                    this.$swal({
        ...
        destroy(id){
            this.$swal({
                ...
                if (result.isConfirmed) {
                    axios.delete(route('priority.destroy', id)).then(resp => {
                        this.table.data = resp.data
                    }).catch(err => {
                        this.$swal({
                            icon: 'error',
